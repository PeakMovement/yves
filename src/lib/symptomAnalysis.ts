...
export function analyzeSymptomLocal(symptomPrompt: string): SymptomAnalysisResult {
  ...
  // Find exact or partial matches in the database
  ...
  for (const entry of SYMPTOM_DATABASE) {
    ...
    if (normalizedPrompt.includes(normalizedSymptom) || normalizedSymptom.includes(normalizedPrompt)) {
      if (entry.symptom.length > (bestMatch?.symptom.length || 0)) {
        bestMatch = entry;
  ...
  // If no direct match, check for keyword overlap
  if (!bestMatch) {
    const promptWords = normalizedPrompt.split(/\s+/);
    for (const entry of SYMPTOM_DATABASE) {
      const symptomWords = entry.symptom.toLowerCase().split(/\s+/);
      const overlap = promptWords.filter((word) => symptomWords.includes(word)).length;

      if (overlap > 2 && entry.score > highestMatchScore) {
        bestMatch = entry;
        highestMatchScore = entry.score;
      }
    }
